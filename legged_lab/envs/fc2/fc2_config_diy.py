...
class bhr8RewardCfg(RewardCfg):
    ...
    body_orientation_l2 = RewTerm(
        func=fc2_r.body_orientation_l2, params={"asset_cfg": SceneEntityCfg("robot", body_names=".*torso.*")}, weight=1.5
    )
...
@configclass
class bhr8FlatEnvCfg(BaseEnvCfg):
    cycle_time: float = 0.8
    reward = bhr8RewardCfg()
    target_joint_pos_scale: float = 0.18
    def __post_init__(self):
        ...
        self.robot.terminate_contacts_body_names = [".*torso.*",".*hip.*",".*shoulder.*",".*bigarm.*"]
...
@configclass
class bhr8FlatAgentCfg(BaseAgentCfg):

    experiment_name: str = "bhr8_flat"
    wandb_project: str = "bhr8_flat"
...
@configclass
class bhr8RoughEnvCfg(bhr8FlatEnvCfg):
...
@configclass
class bhr8RoughAgentCfg(BaseAgentCfg):
    device: str = "cuda:3"
    experiment_name: str = "bhr8_rough"
    wandb_project: str = "bhr8_rough"
    max_iterations: str = 5000
    def __post_init__(self):
        super().__post_init__()
